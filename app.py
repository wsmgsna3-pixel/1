# -*- coding: utf-8 -*-
"""
选股王 · V8.0 精准启动策略：动能 + MACD 趋势共振版
更新说明：
1. 【**策略精调 V8.0**】：核心变动：
   - 目标：修复 V7.0 中 D+3 收益为负的弱点，巩固 D+5 成果。
   - **当日涨幅 (w_pct)** 权重从 0.30 降至 **0.20** (降低纯情绪股的比例)。
   - **MACD (w_macd)** 权重重新引入并设置为 **0.15** (用于过滤掉趋势未启动的股票，确保动能有趋势背景支撑)。
   - **资金流 (w_mf)** 保持 **0.35**。
   - **量比 (w_vol)** 略微削弱至 **0.05**。
   - **所有防御/形态指标 (w_position, w_volatility)** 保持归零。
   
   新权重结构：动能(0.40) + 资金流(0.35) + 趋势(0.15) + 其他(0.10) = 1.00
"""
...
# 页面设置
# ---------------------------
st.set_page_config(page_title="选股王 · V8.0 精准启动策略", layout="wide")
st.title("选股王 · V8.0 精准启动策略（动能 + MACD 趋势共振版）")
st.markdown("✅ **V8.0 策略：削弱纯粹的当日动能 ($\mathbf{w\_pct}=0.20$)，重新引入 $\mathbf{MACD (\text{0.15})}$ 作为趋势共振确认，以修复 D+3 的洗盘弱点。**")
...
with st.sidebar:
    ...
    # V8.0 保持 V7.0 的较低换手率要求，确保有足够初选池
...
def run_backtest_for_a_day(last_trade, TOP_BACKTEST, FINAL_POOL, MIN_PRICE, MAX_PRICE, MIN_TURNOVER, MIN_AMOUNT, MIN_CIRC_MV_BILLIONS):
    ...
    # 🚨 V8.0 精准启动策略：动能 + MACD 趋势共振版
    
    # 核心权重：动能+资金+趋势，占比 90%
    w_pct = 0.20            # 20% - 当日涨幅 (削弱)
    w_mf = 0.35             # 35% - 资金流 (保持核心)
    w_turn = 0.20           # 20% - 换手率 (保持活跃度)
    w_macd = 0.15           # 15% - MACD (**重新引入，趋势共振**)
    
    # 辅助权重：其他动能，占比 10%
    w_vol = 0.05            # 5% - 量比 (辅助)
    w_trend = 0.05          # 5% - 10日回报 (辅助)

    # 彻底归零项
    w_position = 0.00       # 0% - 60日位置 
    w_volatility = 0.00     # 0% - 波动率 
    
    # Sum: 0.20+0.35+0.20+0.15+0.05+0.05 = 1.00
...
if st.button(f"🚀 开始 {BACKTEST_DAYS} 日自动回测"):
    ...
    st.warning("⚠️ **V8.0 版本已更换为动能 + MACD 趋势共振策略，目标是消除 D+3 的洗盘弱点。**")
    ...
    for n in [1, 3, 5]:
        ...
        st.metric(f"Top {TOP_BACKTEST}：D+{n} 平均收益 / 准确率", 
                  ...
                  help=f"总有效样本数：{total_count}。**V8.0 已应用动能 + MACD 趋势共振策略。**")
