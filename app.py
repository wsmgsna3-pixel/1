...
更新说明：
1. 【**策略精调 V4.1f**】：核心变动：
   - 略微提高短期动能（10日回报 w_trend: 0.10 -> 0.15；MACD w_macd: 0.10 -> 0.15），以确保选出“有向上趋势”的股票。
   - 略微降低 60 日位置（w_position: 0.30 -> 0.25）和波动率（w_volatility: 0.20 -> 0.15），允许在安全中低位有更强的启动迹象。
   - 目标：兼顾低位安全和启动动能，寻找处于中低位蓄势，且趋势明确向上的股票。
...
# 页面设置
# ---------------------------
st.set_page_config(page_title="选股王 · V4.1f 低位动能启动版", layout="wide")
st.title("选股王 · V4.1f 低位动能启动版（动能与安全平衡）")
st.markdown("✅ **V4.1f 策略：已平衡“安全低位”和“向上动能”的权重，旨在捕捉低位启动股。**")
...
def run_backtest_for_a_day(last_trade, TOP_BACKTEST, FINAL_POOL, MIN_PRICE, MAX_PRICE, MIN_TURNOVER, MIN_AMOUNT, MIN_CIRC_MV_BILLIONS):
    ...
    # 🚨 V4.1f 策略精调：平衡动能与安全 (30% 动能 + 40% 安全/稳定 + 30% 活跃度)
    
    # 安全/稳定指标：总权重 40%
    w_position = 0.25   # 25% - 60日位置 (低位置得分高 = 核心安全边际)
    w_volatility = 0.15 # 15% - 波动率 (低波动率得分高 = 趋势稳定)
    
    # 趋势/动能/活跃度指标：总权重 60% (其中动能 30%，活跃度 30%)
    w_trend = 0.15      # 15% - 10日回报 (短期动能)
    w_macd = 0.15       # 15% - MACD (趋势方向)
    
    w_turn = 0.10       # 10% - 换手率 (活跃度)
    w_mf = 0.10         # 10% - 资金流 (主力动向)
    w_pct = 0.05        # 5% - 当日涨幅 (右侧启动的第一信号)
    w_vol = 0.05        # 5% - 量比 
    
    # Sum: 0.25+0.15 + 0.15+0.15 + 0.10+0.10+0.05+0.05 = 1.00
...
if st.button(f"🚀 开始 {BACKTEST_DAYS} 日自动回测"):
    ...
    st.warning("⚠️ **V4.1f 版本已更换为低位动能启动策略，请清除 Streamlit 缓存后运行，以使用新的权重计算评分。**")
    ...
    for n in [1, 3, 5]:
        ...
        st.metric(f"Top {TOP_BACKTEST}：D+{n} 平均收益 / 准确率", 
                  ...
                  help=f"总有效样本数：{total_count}。**V4.1f 已应用低位动能启动策略。**")
