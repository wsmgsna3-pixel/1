# -*- coding: utf-8 -*-
"""
选股王 · V9.0 极致胜率策略：启动 + 资金流 + 极致防御版
更新说明：
1. 【**策略精调 V9.0**】：核心变动：
   - 目标：牺牲部分 D+5 爆发力，**极致提升准确率（胜率）**。
   - **60 日位置 (w_position)** 重新引入并设置为 **0.15** (强调安全边际，只选低位启动股)。
   - **波动率 (w_volatility)** 重新引入并设置为 **0.10** (过滤掉已急速拉升过的股票)。
   - **当日涨幅 (w_pct)** 和 **换手率 (w_turn)** 权重都小幅下调，让位给防御指标。
   - **资金流 (w_mf)** 保持 **0.35**，仍是核心启动指标。
   - **量比 (w_vol)** 和 **10日回报 (w_trend)** 权重归零，简化策略结构。
   
   新权重结构：资金流(0.35) + 动能(0.30) + 防御(0.25) + 趋势(0.10) = 1.00
"""
...
# 页面设置
# ---------------------------
st.set_page_config(page_title="选股王 · V9.0 极致胜率策略", layout="wide")
st.title("选股王 · V9.0 极致胜率策略（启动 + 资金流 + 极致防御版）")
st.markdown("✅ **V9.0 策略：重新引入 $\mathbf{60日位置 (\text{0.15})}$ 和 $\mathbf{波动率 (\text{0.10})}$，以牺牲部分爆发力为代价，目标将准确率提升到 $\mathbf{55\%}$ 以上。**")
...
def run_backtest_for_a_day(last_trade, TOP_BACKTEST, FINAL_POOL, MIN_PRICE, MAX_PRICE, MIN_TURNOVER, MIN_AMOUNT, MIN_CIRC_MV_BILLIONS):
    ...
    # ----------------------------------------------------------------------------------
    # 🚨 V9.0 极致胜率策略：启动 + 资金流 + 极致防御版
    
    # 核心权重：资金流，占比 35%
    w_mf = 0.35             # 35% - 资金流 (核心启动指标)

    # 动能权重：当日动能，占比 30%
    w_pct = 0.15            # 15% - 当日涨幅 (小幅削弱)
    w_turn = 0.15           # 15% - 换手率 (小幅削弱)
    
    # 防御权重：安全边际与波动控制，占比 25%
    w_position = 0.15       # 15% - 60日位置 (**重新引入，安全边际**)
    w_volatility = 0.10     # 10% - 波动率 (**重新引入，过滤高风险股**)
    ...
    # Sum: 0.35+0.15+0.15+0.15+0.10+0.10 = 1.00
...
if st.button(f"🚀 开始 {BACKTEST_DAYS} 日自动回测"):
    ...
    st.warning("⚠️ **V9.0 版本已更换为启动 + 资金流 + 极致防御策略，目标是牺牲部分爆发力来最大化胜率。**")
    ...
    for n in [1, 3, 5]:
        ...
        st.metric(f"Top {TOP_BACKTEST}：D+{n} 平均收益 / 准确率", 
                  ...
                  help=f"总有效样本数：{total_count}。**V9.0 已应用启动 + 资金流 + 极致防御策略。**")
