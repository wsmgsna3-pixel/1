...
核心目标：根据 V30.3 失败的教训，在强市中，**放弃 MACD 归一化**，直接使用 MACD **原始绝对值**来捕捉最强烈的趋势动量，力争扭转 D+1 负收益。同时，保持弱市的严格防御过滤机制。

V30.4 优化逻辑：
1. **强市（MACD 绝对优势）：** 仅筛选 MACD > 0 的股票，直接将 MACD 原始值作为主要评分基准（权重 100%），以确保选出市场上绝对趋势最强的股票。
2. **弱市（极致反弹防御）：** 沿用 V30.3 的 MACD/低波平衡模式，并**保留 V28.0 严格的 MA20/60日位置防御过滤**，确保弱市的低风险性。
