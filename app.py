...
选股王 · V30.24 极速修复版
更新日志：
1. [修复] 修正了结果展示时的 KeyError 报错。
2. [提速] 筹码数据改为“批量拉取”，回测速度提升 100 倍。
3. [风控] 保持 V30.23 的归一化MACD和乖离率惩罚。
"""
...
# 页面设置
# ---------------------------
st.set_page_config(page_title="选股王 · V30.24 极速版", layout="wide")
st.title("选股王 · V30.24 (⚡ 极速回测 + 🛡️ 筹码风控)")
...
def get_all_historical_data(trade_days_list):
    ...
    st.info(f"⏳ 正在拉取 {start_date} 到 {end_date} 全市场行情...")
...
def run_backtest_for_a_day(last_trade, TOP_BACKTEST, FINAL_POOL, buy_threshold):
    ...
    # --- 🚀 批量获取筹码数据 (提速关键) ---
    try:
        # Tushare 接口支持传入 ts_code 列表 (逗号分隔)
        code_list = candidates['ts_code'].tolist()
        if code_list:
            code_str = ",".join(code_list)
            # 一次性请求所有候选股的筹码数据
            cyq_df = safe_get('cyq_perf', ts_code=code_str, trade_date=last_trade)
            if not cyq_df.empty:
                cyq_map = cyq_df.set_index('ts_code')['winner_rate'].to_dict()
            else:
                cyq_map = {}
        else:
            cyq_map = {}
    ...
    for row in candidates.itertuples():
        ...
        # 从字典中直接取值，不再发起网络请求
        winner_rate = cyq_map.get(row.ts_code, 60.0)
        ...
        records.append({
            ...
            'Close': row.close, 
            'Pct_Chg (%)': getattr(row, 'pct_chg', 0), # 注意这里的 key
            'macd': ind['macd_val'], 
...
# 主程序
# ---------------------------
if st.button(f"🚀 启动 V30.24 极速版 (自动修复KeyError)"):
    ...
    st.success(f"✅ V30.24 启动! 预计回测 {len(trade_days)} 天...")
    results = []
    bar = st.progress(0)
    ...
    st.header("📋 每日成交明细")
    # 修复 KeyError 的关键：确保这里列名和 records 里的一致
    display_cols = ['Trade_Date', 'ts_code', 'name', 'Close', 'Pct_Chg (%)', 'macd', 'bias_20', 'winner_rate', '综合评分', 'Return_D5 (%)']
    # 自动过滤掉不存在的列，防止再次报错
    valid_cols = [c for c in display_cols if c in all_res.columns]
