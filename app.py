...
# ---------------------------
# 2. 数据获取与合成 (修复了 _pro 报错)
# ---------------------------
@st.cache_data(ttl=3600*12)
def safe_get(_pro, func_name, **kwargs):
    """带缓存的安全获取数据函数，_pro 避免了 Streamlit 的 Hash 报错"""
...
if st.button("🚀 开始扫描并统计近50周翻倍股票"):
    ...
    st.write(f"🔍 符合初始过滤条件（价格≥{MIN_PRICE}元，市值{MIN_MV}-{MAX_MV}亿）的股票共 **{len(filtered_stocks)}** 只，开始分析周线走势...")
