...
2. [主升浪黄金段锁定] 严格筛选洗盘次数在 2 到 5 次之间的标的，精准捕捉最具爆发力的第 3 浪主升段。
3. [阶梯加权评分] 处于第 2-3 次洗盘（主升前夜）的标的给予高权重加分，优先排序。
...
CACHE_FILE_NAME = "market_data_cache_v40_0.pkl" 
...
# 页面设置
# ---------------------------
st.set_page_config(page_title="选股王 V40.0 主升浪波浪动能版", layout="wide")
st.title("选股王 V40.0：周线MACD波浪动能识别 + 主升浪黄金段阶梯加权")
...
def compute_trend_indicators(ts_code, end_date, use_sina=False, _run_id=None):
    ...
    if res['is_v38_buy_signal']:
        ...
        res['wave_count'] = wave_count  # 记录洗盘次数，用于后续加权评分
...
def run_backtest_for_a_day(last_trade, TOP_BACKTEST, MIN_MV, MAX_MV, MIN_PRICE, use_sina=False, run_timestamp=None):
    ...
    for row in df.itertuples():
        ...
        # 【V40阶梯加权评分】：洗盘 2-3 次（主升爆发前夜）给予额外 +30 分权重奖励，优先排在最前列
        wave_cnt = ind.get('wave_count', 3)
        if wave_cnt in [2, 3]:
            total_score += 30.0  # S级主升段加权奖励
...
with st.sidebar:
    ...
    CHECKPOINT_FILE = "backtest_checkpoint_v40_0_wave.csv" 
...
if st.button(f"🚀 启动 V40.0 主升浪波浪追踪"):
    SINA_STATUS = {'success': 0, 'fail': 0}
    processed_dates = set()
    results = []
    ...
    if results:
        ...
        st.download_button("📥 下载完整轨迹 (CSV)", csv, f"export_v40_0_wave.csv", "text/csv")
    else:
