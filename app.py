# -*- coding: utf-8 -*-
"""
选股王 ｖ1.0日线版 · V40.4日线扩展诊断版
============================================================
主要变化
1. 股票池：主板/创业板/科创板，明确剔除北交所；并行比较原200亿元与扩池100亿元下限。
...
9. 数据：改用逐交易日分片缓存；每成功下载一天立即原子保存，网络中断后只补缺失日期。
10. 诊断：输出逐日筛选漏斗、退出周、持仓天数、MFE/MAE等字段。
11. 门槛：固定保留V40.4原策略的82分准入，不再计算80分、78分和仅排序实验模式。
12. 环境：取消创业板指/科创50的一刀切暂停开仓，避免误杀弱市中的强势股。
13. 保本：浮盈首次达到10%后，从下一交易日启用成本上方0.3%的保护止损。
14. 审计：另行导出全部核心候选及未入选原因，便于核查质量门槛。
15. 股票池对照：基准/单只模式为20元、200~1000亿元；扩池模式为10元、100~1000亿元。
16. 广度对照：基准保留至少2只确认；单只和扩池模式允许当天仅1只高质量股票入选。
17. 组合：30万元初始资金、最多3只持仓、单只约10万元，输出组合曲线与真实空仓率。
18. 断点：逐日分析采用参数隔离、主键去重和原子覆盖，下载与分析都可中断续跑。
19. 周线字段：保留V40.4原有周线MACD诊断和G1/R5弱阶段剔除，不增加新的红柱理论条件。
...
27. 评分精简：只保留固定82分的V40.4基准策略，删除80分、78分和仅排序的
    计算、页面、漏斗与导出内容，减少无效诊断开销。
28. 基准保留：V40.4原始20元/200亿元、至少2只确认、固定8周模式完整保留。
29. 信号扩容：并行增加“允许单只”和“10元/100亿元扩池”两种信号模式；
    两者仍保持82分、T+1可成交、TopK和原始单股质量条件。
30. 日线延长：每种信号模式并行对照固定8周、强势延长12周、强势延长16周；
    只有W8/W12仍盈利、收盘在日MA20上方且MA20未下降时才继续持有。
31. 延长风控：延长阶段继续执行原结构止损、成本保护和移动止盈；
    连续2个交易日收盘低于日MA20时，下一交易日开盘退出。
32. 缺失容忍：行情下载失败时自动跳过无法分析的信号日，不再中止整个回测。
33. 下载修复：结果区在当前会话保持可见，下载按钮不触发页面重新运行。
...
# 经多轮A/B回测确认后固化的V40.4基准参数。
BASE_MIN_PRICE = 20.0
BASE_MIN_MV = 200.0
EXPANDED_MIN_PRICE = 10.0
EXPANDED_MIN_MV = 100.0
FIXED_MAX_MV = 1000.0
FIXED_MARKET_FILTER = False
BASE_MIN_TRADABLE_SIGNALS = 2
...
# 三种信号模式只改变股票池和按日最少信号数，单股82分质量线保持一致。
SIGNAL_MODES = (
    ("V40.4基准信号", "Mode_Base", 82.0, BASE_MIN_PRICE, BASE_MIN_MV, 2),
    ("日线单只信号", "Mode_Single", 82.0, BASE_MIN_PRICE, BASE_MIN_MV, 1),
    ("日线扩池信号", "Mode_Expanded", 82.0, EXPANDED_MIN_PRICE, EXPANDED_MIN_MV, 1),
)

# 固定8周是原始基准；12/16周只允许日线趋势仍完整的盈利仓位延长。
HOLDING_POLICIES = (
    ("固定8周", "H8", 8, False),
    ("强势延长12周", "H12", 12, True),
    ("强势延长16周", "H16", 16, True),
)
MAX_REPORT_WEEKS = max(item[2] for item in HOLDING_POLICIES)
STRATEGY_MODES = tuple(
    (
        f"{signal_label}｜{holding_label}",
        signal_label,
        prefix,
        threshold,
        min_price,
    ...
    for signal_label, prefix, threshold, min_price, min_mv, min_signals in SIGNAL_MODES
    for holding_label, holding_key, max_weeks, conditional_extension in HOLDING_POLICIES
)
SCORE_MODE_LABELS = tuple(item[0] for item in STRATEGY_MODES)
...
# 页面
# -----------------------------------------------------------------------------
st.set_page_config(page_title=f"选股王 {VERSION}", layout="wide")
st.title(f"选股王 {VERSION}：V40.4日线扩展诊断")
...
def load_legacy_market_cache(required: set[str]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for path in legacy_cache_paths():
        ...
        try:
            ...
            if not isinstance(daily.index, pd.MultiIndex) or not isinstance(adj.index, pd.MultiIndex):
                daily, adj = standardize_market_frames(daily, adj)
            return filter_market_dates(daily, required), filter_market_dates(adj, required)
        except Exception as exc:
...
def load_market_data(required_dates: list[str], use_cache: bool = True) -> list[str]:
    ...
    # 只有行情和复权因子同时存在才视为完整缓存，避免半包数据污染回测。
    ...
    if missing_dates:
        ...
        # 顺序下载避免触发Tushare频率限制；成功一天就立即落盘一天。
    ...
    result.update(
        {
            ...
            "Holding_Policy": "固定8周",
            "Max_Hold_Weeks": int(hold_weeks),
            "Extended_From_W8": False,
            "Extended_From_W12": False,
            "Extension_Exit": False,
            "Tradable": True,
        }
    )
    return result
...
def get_medium_term_future(
    ...
    holding_policy: str = "固定8周",
    use_sina: bool = False,
) -> dict:
    d0 = datetime.strptime(selection_date, "%Y%m%d")
    start_date = (d0 - timedelta(days=70)).strftime("%Y%m%d")
    # 16周约需112个自然日；额外预留节假日和停牌缓冲。
    end_date = (d0 + timedelta(days=max(110, int(hold_weeks) * 10))).strftime("%Y%m%d")
    hist = get_qfq_data(ts_code, start_date, end_date, use_sina=use_sina)
    result = future_result_template(hold_weeks)
    result["Holding_Policy"] = holding_policy
    result["Max_Hold_Weeks"] = int(hold_weeks)
    if hist.empty:
        result["Exit_Reason"] = "未来行情缺失"
        return result
    hist = hist.copy()
    hist["ma20_hold"] = pd.to_numeric(hist["close"], errors="coerce").rolling(20).mean()
    hist["ma20_hold_prev"] = hist["ma20_hold"].shift(1)
    ...
    for i in range(observed_days):
        ...
        # W8之后只保留日线趋势仍完整的盈利仓位；连续两日跌破MA20次日退出。
        if conditional_extension and day_count > 40:
            ma20_now = pd.to_numeric(pd.Series([row.get("ma20_hold", np.nan)]), errors="coerce").iloc[0]
            if pd.notna(ma20_now) and curr_close < float(ma20_now):
                below_ma20_streak += 1
            else:
                below_ma20_streak = 0
            if below_ma20_streak >= 2:
                pending_reason = "延长阶段连续2日跌破MA20-次日开盘退出"
        ...
        # 第8周和第12周只在收盘后使用当时已知的日线状态决定是否延长。
        if conditional_extension and day_count in {40, 60} and day_count < hold_weeks * 5:
            checkpoint_week = day_count // 5
            ma20_now = pd.to_numeric(pd.Series([row.get("ma20_hold", np.nan)]), errors="coerce").iloc[0]
            ma20_prev = pd.to_numeric(
                pd.Series([row.get("ma20_hold_prev", np.nan)]), errors="coerce"
            ).iloc[0]
            checkpoint_return = net_return(curr_close)
            trend_ok = bool(
                checkpoint_return > 0.0
                and pd.notna(ma20_now)
                and pd.notna(ma20_prev)
                and curr_close >= float(ma20_now)
                and float(ma20_now) >= float(ma20_prev)
            )
            if not trend_ok:
                result["Extension_Exit"] = True
                finalize_exit(
                    curr_close,
                    checkpoint_week,
                    day_count,
                    f"W{checkpoint_week}日线趋势未通过-周期收盘退出",
                    row.name,
                )
                result[f"Held_W{checkpoint_week}"] = True
                exited = True
                break
            if checkpoint_week == 8:
                result["Extended_From_W8"] = True
            elif checkpoint_week == 12:
                result["Extended_From_W12"] = True
...
def apply_score_modes(
    ...
    prices = pd.to_numeric(all_candidates["Signal_Close"], errors="coerce")
    market_values = pd.to_numeric(all_candidates["circ_mv"], errors="coerce")

    for label, prefix, threshold, min_price, min_mv, min_tradable_signals in SIGNAL_MODES:
        pool_eligible = (
            prices.ge(float(min_price))
            & market_values.between(float(min_mv), float(FIXED_MAX_MV), inclusive="both")
        ).fillna(False)
        score_eligible = pool_eligible & scores.ge(float(threshold)).fillna(False)
        ...
        all_candidates[f"{prefix}_Pool_Eligible"] = pool_eligible
        ...
        all_candidates[status_col] = "不在本模式股票池"
        all_candidates.loc[pool_eligible & ~score_eligible, status_col] = "未达到82分准入"
        ...
        funnel[f"{prefix}_Pool_Eligible"] = int(pool_eligible.sum())
    ...
    # 旧字段固定映射到V40.4基准信号模式，保证原漏斗口径可继续对照。
    funnel["Base_Score_Pass"] = funnel["Mode_Base_Score_Eligible"]
    funnel["Dynamic_Score_Pass"] = funnel["Mode_Base_Score_Eligible"]
    funnel["Final_Signal"] = funnel["Mode_Base_Score_Eligible"]
    funnel["Tradable_Signal"] = funnel["Mode_Base_Tradable"]
    funnel["Breadth_Confirmed"] = funnel["Mode_Base_Breadth_Confirmed"]
    funnel["Scarce_Tradable_Skipped"] = funnel["Mode_Base_Scarce_Skipped"]
    funnel["Selected_TopK"] = funnel["Mode_Base_Selected_TopK"]
...
def collect_mode_results(
    ...
    """把三种信号与三种持有规则展开成九组可直接比较的策略结果。"""
    rows: list[dict] = []
    future_cache: dict[tuple[int, str], dict] = {}
    for signal_label, prefix, threshold, min_price, min_mv, min_signals in SIGNAL_MODES:
        selected = all_candidates[f"{prefix}_Selected"].fillna(False).astype(bool)
        if not selected.any():
            continue
        for idx in all_candidates.index[selected]:
            candidate = all_candidates.loc[idx]
            for holding_label, holding_key, max_weeks, conditional_extension in HOLDING_POLICIES:
                cache_key = (int(idx), holding_key)
                if cache_key not in future_cache:
                    if max_weeks == 8 and not conditional_extension:
                        future_cache[cache_key] = {
                            key: candidate.get(key, default)
                            for key, default in future_result_template(8).items()
                        }
                    else:
                        future_cache[cache_key] = get_medium_term_future(
                            ...
                            protection_trigger_pct=protection_trigger_pct,
                            protection_buffer_pct=protection_buffer_pct,
                            conditional_extension=bool(conditional_extension),
                            holding_policy=holding_label,
                            use_sina=use_sina,
                        )
                row = candidate.to_dict()
                row.update(future_cache[cache_key])
                row["Diagnostic_Mode"] = f"{signal_label}｜{holding_label}"
                row["Signal_Mode"] = signal_label
                row["Holding_Policy"] = holding_label
                row["Max_Hold_Weeks"] = int(max_weeks)
                row["Mode_Score_Threshold"] = float(threshold)
                row["Mode_Min_Price"] = float(min_price)
                row["Mode_Min_MV"] = float(min_mv)
                row["Mode_Min_Tradable_Signals"] = int(min_signals)
                row["Rank"] = candidate.get(f"{prefix}_Rank", pd.NA)
                row["Selection_Status"] = candidate.get(f"{prefix}_Status", "")
                rows.append(row)
    if not rows:
        return pd.DataFrame()
...
def run_backtest_for_day(
    ...
    for key in [
        ...
        funnel[key] = 0
    for _, prefix, _, _, _, _ in SIGNAL_MODES:
        for suffix in [
            "Pool_Eligible", "Score_Eligible", "Tradable", "Breadth_Confirmed",
    ...
    # 固定使用V40.4原始82分门槛；低于82分的候选只保留D0审计字段。
    ...
    all_candidates["Selection_Status"] = "核心候选-等待82分准入"
    ...
    if regime["Market_Weak_Block"]:
        ...
        # 80/78/仅排序实验已删除：82分以下候选不再计算T+1和未来8周轨迹。
        # Total_Score及82分门槛均为D0已知，因此不会改变V40.4基准入选结果。
        ...
        funnel["Time_Future_82_S"] = round(time.perf_counter() - future_started, 3)
    ...
    funnel["Full_Future_Count"] = int(all_candidates["Execution_Checked"].fillna(False).sum())
    funnel["Time_Total_S"] = round(time.perf_counter() - day_started, 3)

    # 旧通用列映射到82分模式，候选CSV仍可与V40.3直接对照。
    all_candidates["Day_Tradable_Signal_Count"] = all_candidates["Mode_Base_Day_Tradable_Count"]
    all_candidates["Breadth_Confirmed"] = all_candidates["Mode_Base_Breadth_Confirmed"]
    all_candidates["Selected"] = all_candidates["Mode_Base_Selected"]

    internal_cols = ["_Signal_Low", "_Signal_MA20", "_ATR14"]
    final = collect_mode_results(
        all_candidates,
        max_gap_pct=max_gap_pct,
        buy_slippage_pct=buy_slippage_pct,
        sell_slippage_pct=sell_slippage_pct,
...
def show_weekly_report(all_results: pd.DataFrame) -> None:
    ...
    for row_start in range(1, MAX_REPORT_WEEKS + 1, 4):
        metric_columns = st.columns(4)
        for offset, target in enumerate(metric_columns):
            week = row_start + offset
            if week > MAX_REPORT_WEEKS:
                continue
            ret_col = f"Return_W{week} (%)"
            eligible_col = f"Eligible_W{week}"
            held_col = f"Held_W{week}"
            with target:
                if eligible_col not in all_results.columns:
                    st.metric(f"W{week}", "本模式不适用")
                    continue
...
def signal_gap_metrics(signals: pd.DataFrame, trade_days: list[str]) -> dict:
    """按各模式已经完成自身广度确认的最终入选日期统计空窗。"""
    days = sorted({normalize_date(day) for day in trade_days if normalize_date(day)})
    signal_dates = set()
    if signals is not None and not signals.empty and "Trade_Date" in signals.columns:
        signal_dates = set(signals["Trade_Date"].map(normalize_date))
...
def build_mode_comparison(
    ...
    for label in SCORE_MODE_LABELS:
        ...
        mature_mask = realized.notna()
        mature = data.loc[mature_mask].copy()
        mature_returns = realized.loc[mature_mask]
        exit_reason = mature.get(
            "Exit_Reason", pd.Series("", index=mature.index)
        ).fillna("").astype(str)
        gaps = signal_gap_metrics(data, trade_days)
        summary = portfolio_summaries.get(label, {})
        extended_w8 = (
            int(bool_series(data["Extended_From_W8"]).sum())
            if "Extended_From_W8" in data.columns else 0
        )
        extended_w12 = (
            int(bool_series(data["Extended_From_W12"]).sum())
            if "Extended_From_W12" in data.columns else 0
        )
        row = {
            "模式": label,
            "信号模式": data["Signal_Mode"].iloc[0] if len(data) and "Signal_Mode" in data.columns else "",
            "持有规则": data["Holding_Policy"].iloc[0]
            if len(data) and "Holding_Policy" in data.columns else "",
            "入选样本": len(data),
            "成熟样本": len(mature),
            "信号日": gaps["Signal_Days"],
            "超过10日空窗次数": gaps["Gaps_Over_10"],
            "最长空窗(交易日)": gaps["Longest_Gap_Days"],
            "平均实现收益(%)": round(mature_returns.mean(), 2) if len(mature) else np.nan,
            "亏损率(%)": round((mature_returns < 0).mean() * 100.0, 1) if len(mature) else np.nan,
            "结构止损率(%)": round(exit_reason.str.startswith("结构止损").mean() * 100.0, 1)
            if len(mature) else np.nan,
            "假反弹率(%)": round(bool_series(mature["False_Rebound_10D"]).mean() * 100.0, 1)
            if len(mature) and "False_Rebound_10D" in mature.columns else np.nan,
            "平均持仓天数": round(pd.to_numeric(mature.get("Holding_Days"), errors="coerce").mean(), 1)
            if len(mature) else np.nan,
            "W8延长数": extended_w8,
            "W12延长数": extended_w12,
            ...
            "组合平均持仓": round(summary.get("Average_Positions", np.nan), 2),
            "组合交易数": summary.get("Executed_Entries", 0),
            "三仓已满未买": summary.get("Rejected_Full", 0),
        }
        for week in range(1, MAX_REPORT_WEEKS + 1):
            eligible_col = f"Eligible_W{week}"
            held_col = f"Held_W{week}"
...
def show_mode_comparison(comparison: pd.DataFrame) -> None:
    st.subheader("🧪 ｖ1.0日线版九模式总览")
    st.caption(
        "三种信号口径 × 三种持有期限；全部保持82分、T+1成交、TopK和30万元三仓。"
    )
...
with st.sidebar:
    ...
    st.subheader("并行股票池与交易约束")
    # 逐日计算必须覆盖最宽股票池；各模式在82分准入时再按自己的股票池切分。
    MIN_PRICE = EXPANDED_MIN_PRICE
    MIN_MV = EXPANDED_MIN_MV
    MAX_MV = FIXED_MAX_MV
    ENABLE_MARKET_FILTER = FIXED_MARKET_FILTER
    st.info(
        "基准：20元/200亿元且至少2只；单只版：20元/200亿元且允许1只；"
        "扩池版：10元/100亿元且允许1只。三种模式上限均为1000亿元。"
    )
    st.caption("组合：初始30万元，最多3只，单只目标约10万元。")
    st.info(
        "三种信号模式都保留V40.4原始82分准入；"
    ...
    DETAIL_MODE = st.selectbox(
        ...
        index=0,
        help="总览始终比较全部九组；这里只决定页面下方展开哪一组明细。",
    )
    ...
    st.caption("固定82分准入；评分公式、排序和V40.4其他硬门槛保持不变。")
    ...
    USE_CACHE = st.checkbox("使用逐交易日分片行情缓存", value=True)
    st.caption("每成功下载一个交易日立即原子保存；中断后只补缺失日期，并复用周线1.0缓存。")
    if st.button("清除全部共享行情缓存"):
...
run_clicked = st.button(f"🚀 启动 {VERSION} 科技增强回测")
if run_clicked:
    st.session_state["v1_daily_result_visible"] = True

# 下载按钮使用ignore避免重跑；会话标记保证其他普通页面重跑后结果区仍然存在。
if st.session_state.get("v1_daily_result_visible", False):
    API_ERRORS.clear()
    SINA_STATUS = {"success": 0, "fail": 0}
    ...
    try:
        ...
        theoretical_end = datetime.strptime(latest_signal, "%Y%m%d") + timedelta(
            days=max(110, MAX_REPORT_WEEKS * 10)
        )
        ...
        failed_set = set(market_download_failures)
        skipped_signal_days = [day for day in trade_days if day in failed_set]
        if market_download_failures:
            st.warning(
                f"行情仍缺失 {len(market_download_failures)} 个交易日；"
                f"其中回测信号日 {len(skipped_signal_days)} 个已自动跳过，其余日期继续分析。"
            )
        trade_days = [day for day in trade_days if day not in failed_set]
        if not trade_days:
            raise RuntimeError("本次回测信号日全部缺失，暂无可分析日期")
            ...
            "signal_modes": [
                {
                    ...
                    "min_price": min_price,
                    "min_mv": min_mv,
                    "min_tradable_signals": min_signals,
                }
                for label, _, threshold, min_price, min_mv, min_signals in SIGNAL_MODES
            ],
            "holding_policies": [
                {
                    "label": label,
                    "max_weeks": max_weeks,
                    "conditional_extension": conditional_extension,
                }
                for label, _, max_weeks, conditional_extension in HOLDING_POLICIES
            ],
        ...
        if RESUME_CHECKPOINT and os.path.exists(state_file):
            try:
                ...
                if "Error" in funnel_store.columns:
                    error_dates = set(
                        funnel_store.loc[funnel_store["Error"].notna(), "Trade_Date"].astype(str)
                    )
                    processed -= error_dates
        ...
        st.subheader("🧭 三种82分信号模式逐日漏斗")
        if not funnel_df.empty:
            mode_funnel_rows = []
            for label, prefix, threshold, min_price, min_mv, min_signals in SIGNAL_MODES:
                for suffix in [
                    "Pool_Eligible", "Score_Eligible", "Tradable", "Breadth_Confirmed",
                    "Scarce_Skipped", "Selected_TopK",
                ]:
                ...
                mode_funnel_rows.append(
                    {
                        "模式": label,
                        "分数门槛": threshold,
                        "价格/市值下限": f"{min_price:.0f}元/{min_mv:.0f}亿",
                        "当日最少可成交数": min_signals,
                        "股票池内核心候选": int(pd.to_numeric(
                            funnel_df[f"{prefix}_Pool_Eligible"], errors="coerce"
                        ).fillna(0).sum()),
                        ...
                        "满足本模式数量日期": int(pd.to_numeric(
                            funnel_df[f"{prefix}_Breadth_Confirmed"], errors="coerce"
                        ).fillna(0).sum()),
                        "数量不足放弃日期": int((pd.to_numeric(
        ...
        if not results_store.empty:
            ...
            st.download_button(
                ...
                f"weekly_summary_{EXPORT_TAG}_{config_hash}.csv",
                "text/csv",
                on_click="ignore",
            )
            st.download_button(
                ...
                f"weekly_signals_{EXPORT_TAG}_{config_hash}.csv",
                "text/csv",
                on_click="ignore",
            )
            if not portfolio_curve.empty:
                st.download_button(
                    ...
                    f"portfolio_curve_{EXPORT_TAG}_{config_hash}.csv",
                    "text/csv",
                    on_click="ignore",
                )
            if not portfolio_ledger.empty:
                st.download_button(
                    ...
                    f"portfolio_ledger_{EXPORT_TAG}_{config_hash}.csv",
                    "text/csv",
                    on_click="ignore",
                )
            if not portfolio_orders.empty:
                st.download_button(
                    ...
                    f"portfolio_orders_{EXPORT_TAG}_{config_hash}.csv",
                    "text/csv",
                    on_click="ignore",
                )
        else:
            st.warning("本次九种策略均未产生可交易信号，请先查看筛选漏斗。")

        if not all_candidates_export.empty:
            candidate_csv = all_candidates_export.to_csv(index=False).encode("utf-8-sig")
            st.download_button(
                "📥 下载ｖ1.0日线版全部核心候选诊断CSV",
                candidate_csv,
                f"candidates_{EXPORT_TAG}_{config_hash}.csv",
                "text/csv",
                on_click="ignore",
            )
        if not funnel_df.empty:
            ...
            st.download_button(
                "📥 下载ｖ1.0日线版逐日筛选漏斗CSV",
                funnel_csv,
                f"funnel_{EXPORT_TAG}_{config_hash}.csv",
                "text/csv",
                on_click="ignore",
            )
