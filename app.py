# -*- coding: utf-8 -*-
"""
选股王 · V3.9.16 最终诊断增强版（表格新增关键字段）
更新说明：
1. 【**诊断增强 V3.9.16**】：在每日回测详情表格中，新增 '股价'、'涨跌幅' 和 '流通市值'，方便快速发现数据异常和策略偏好。
2. 【**计算修正**】：将 'selection_price' 重命名为 'Close'，更符合股票数据习惯。
"""
...
# (此处省略 V3.9.15 中未改动的代码，包括辅助函数、get_future_prices、compute_indicators、侧边栏参数和 Token 初始化部分)
# 请确保你的脚本这部分与 V3.9.15 保持一致！
# --------------------------------------------------------------------------------------
# (请将 V3.9.15 脚本的这部分代码粘贴到这里，以确保完整性)
# --------------------------------------------------------------------------------------
# ... (辅助函数、get_trade_days, safe_bar_get, get_future_prices, compute_indicators, 侧边栏, Token 初始化) ...
# ... (保持 V3.9.15 相同) ...


# ---------------------------
# 核心回测逻辑函数 - V3.9.16 (修改数据收集和输出)
# ---------------------------
def run_backtest_for_a_day(last_trade, TOP_BACKTEST, FINAL_POOL, MIN_PRICE, MAX_PRICE, MIN_TURNOVER, MIN_AMOUNT):
    """为单个交易日运行选股和回测逻辑 (V3.9.16 修正)"""
    ...
    # 2. 合并基本面数据
    stock_basic = safe_get('stock_basic', list_status='L', fields='ts_code,name,list_date') # 增加 list_date
    ...
    for row in final_candidates.itertuples():
        ...
        # 调用 compute_indicators (计算指标和获取复权买入价)
        ...
        # 调用 get_future_prices (获取未来复权收盘价)
...
if st.button(f"🚀 开始 {BACKTEST_DAYS} 日自动回测"):
    ...
    for n in [1, 3, 5]:
        col = f'Return_D{n} (%)' # 注意这里列名已改为带百分号
