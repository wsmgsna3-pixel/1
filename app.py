...
说明：
- 【本次修复】**已移除所有非法标记字符**（SyntaxError）。
- **标题精简**为“选股王（极速版）”，优化移动端显示。
- **回测缓存修复**已在上一版本完成。
"""
...
st.markdown("## 选股王（极速版）")
st.markdown("输入你的 Tushare Token（输入后按回车）。若有权限缺失，脚本会自动降级并继续运行。")
...
with st.sidebar:
    st.header("可调参数（实时）")
    # Tushare 接口字段的默认值参考 crjsb.py.txt
...
""")
    st.info("若回测仍有中断，请将 Streamlit 屏幕上的任何报错信息完整复制给我。")
