...
R16_LIFECYCLE_HORIZONS = (3, 4, 6, 8)
...
R16_EXIT_RULES = (
    ...
    (
        "日内-8%硬止损（敏感性）",
        "hard",
        "R16_Stop_Minus8_Triggered",
        "R16_Stop_Minus8_Return_Net_pct",
        "R16_Stop_Minus8_Exit_Day",
        None,
    ),
    (
        R16_PRIMARY_EXIT_RULE,
        "hard",
        "R16_Stop_Minus10_Triggered",
        "R16_Stop_Minus10_Return_Net_pct",
        "R16_Stop_Minus10_Exit_Day",
        None,
    ),
    (
        "W1收盘亏损",
        "weekly",
        "R14_Trigger_W1_Close_Loss",
...
def score_r13_candidates(pool_snapshots: pd.DataFrame):
    ...
    # R15不新增强势公式，只冻结并复用R11温和ATR Top1。独立字段用于
    # 影子组合审计，禁止覆盖Selected_Top2或以第二名递补。
    candidates["R15_Strong_ATR_Top1"] = _bool_series(
        candidates, "R11_Strong_Research_Top1"
    )
    ...
    candidates["R15_Shadow_Selected"] = (
        _bool_series(candidates, "Selected_Top2")
        | _bool_series(candidates, "R15_Strong_ATR_Top1")
    )
    # R16只建立独立退出研究样本；不改写任何冻结入场或实际组合字段。
    candidates["R16_Exit_Research_Selected"] = _bool_series(
        candidates, "R15_Shadow_Selected"
    )
...
def track_fixed_future_path(
    ...
    for stop_label in ("Minus8", "Minus10"):
        result[f"R16_Stop_{stop_label}_Triggered"] = False
        result[f"R16_Stop_{stop_label}_Trigger_Date"] = None
        result[f"R16_Stop_{stop_label}_Trigger_Day"] = np.nan
        result[f"R16_Stop_{stop_label}_Exit_Date"] = None
        result[f"R16_Stop_{stop_label}_Exit_Day"] = np.nan
        result[f"R16_Stop_{stop_label}_Exit_Price_QFQ"] = np.nan
        result[f"R16_Stop_{stop_label}_Return_Net_pct"] = np.nan
        result[f"R16_Stop_{stop_label}_Delay_Days"] = np.nan
    ...
    # R16硬止损采用实盘保守成交：买入日不触发（A股T+1），从次一交易日
    # 开始检查；盘中触线按止损价并计0.3%不利滑点，跳空越过止损按开盘价；
    # 停牌或一字跌停不虚构成交，触发后持续挂单直至首个可交易开盘。
    def _simulate_r16_hard_stop(stop_loss_pct: float):
        stop_price = buy_price * (1.0 + stop_loss_pct / 100.0)
        ...
        for position, (_, stop_row) in enumerate(future.iterrows()):
            ...
            if position == 0:  # 买入当日即使盘中跌破也受T+1限制，不能卖出。
            ...
            hit_stop = math.isfinite(day_low) and day_low <= stop_price
            if trigger_position is None and hit_stop:
            ...
            if trigger_position is not None:
                if one_price_limit_down:
                    ...
                    return {
                        ...
                        "Trigger_Day": trigger_position + 1,
                        "Exit_Date": str(future.index[position]),
                        "Exit_Day": position + 1,
                        ...
                        "Delay_Days": position - trigger_position,
        ...
        return {
            "Triggered": trigger_position is not None,
            "Trigger_Date": trigger_date,
            "Trigger_Day": (
                trigger_position + 1 if trigger_position is not None else np.nan
            ),
            "Exit_Date": None,
            "Exit_Day": np.nan,
            "Exit_Price_QFQ": np.nan,
            "Return_Net_pct": np.nan,
            "Delay_Days": np.nan,
            "Blocked_Days": blocked_days,
        }

    for stop_loss_pct, stop_label in (
        (R16_SENSITIVITY_STOP_PCT, "Minus8"),
        (R16_PRIMARY_STOP_PCT, "Minus10"),
    ):
        stop_result = _simulate_r16_hard_stop(stop_loss_pct)
        for field, value in stop_result.items():
            result[f"R16_Stop_{stop_label}_{field}"] = value
    ...
    result["R16_Lifecycle_Data_Available"] = bool(primary_complete)
        ...
        rows.append(
            {
                ...
                "R16_Exit_Research_Selected": bool(
                    candidate_row is not None
                    and _bool_series(
                        pd.DataFrame([candidate_row]),
                        "R16_Exit_Research_Selected",
                    ).iloc[0]
                ),
...
# -----------------------------------------------------------------------------
# R15 冻结R11强势Top1与R3/R6影子组合审计
# -----------------------------------------------------------------------------
def _r15_strong_rows(history: pd.DataFrame, complete_only: bool = False):
    if history.empty:
        return history.iloc[0:0].copy()
    mask = _bool_series(history, "R15_Strong_ATR_Top1")
    if complete_only:
        mask &= _bool_series(history, "Outcome_Complete")
        mask &= _bool_series(history, "Entry_Tradable")
    return history[mask].copy()


def _r15_performance_row(label: str, group: pd.DataFrame):
    returns = pd.to_numeric(
        group.get(PRIMARY_RETURN_COLUMN, pd.Series(dtype=float)), errors="coerce"
    ).dropna()
    valid = group.loc[returns.index].copy() if len(returns) else group.iloc[0:0].copy()
    ...
    return {
        "R15影子组合组别": label,
        "完整交易": len(returns),
        "完整信号周": valid["Signal_Date"].nunique() if len(valid) else 0,
        "胜率%": (returns > 0.0).mean() * 100.0 if len(returns) else np.nan,
        "中位收益%": returns.median() if len(returns) else np.nan,
        "平均收益%": returns.mean() if len(returns) else np.nan,
        "Profit_Factor": _profit_factor(returns),
...
def r15_shadow_performance_audit(history: pd.DataFrame):
    columns = [
        "R15影子组合组别", "完整交易", "完整信号周", "胜率%",
        "中位收益%", "平均收益%", "Profit_Factor",
        "去最佳一只平均收益%", "去最佳一只PF", "最佳一只占正利润%",
    ]
    if history.empty:
        return pd.DataFrame(columns=columns)
    complete = _bool_series(history, "Outcome_Complete") & _bool_series(
        history, "Entry_Tradable"
    )
    actual = history[complete & _bool_series(history, "Selected_Top2")].copy()
    strong = history[
        complete & _bool_series(history, "R15_Strong_ATR_Top1")
    ].copy()
    shadow = history[
        complete
        & (
    ...
    return pd.DataFrame(
        [
            _r15_performance_row("R3/R6原实际基线", actual),
            _r15_performance_row("R15强势温和ATR Top1", strong),
            _r15_performance_row("R3/R6+R15强势影子组合", shadow),
        ],
        columns=columns,
    )


def r15_signal_coverage_audit(history: pd.DataFrame, ledger: pd.DataFrame):
    columns = [
        "市场状态", "扫描周", "R3/R6实际信号周", "R15新增强势信号周",
        "影子组合信号周", "影子组合新信号覆盖率%", "影子组合无新信号周",
    ]
    if history.empty or ledger.empty:
        return pd.DataFrame(columns=columns)
    frame = ledger.copy()
    frame["Signal_Date"] = frame["Signal_Date"].map(parse_yyyymmdd)
    frame = frame.dropna(subset=["Signal_Date"]).sort_values("Signal_Date")
    history_dates = history.copy()
    history_dates["Signal_Date"] = history_dates["Signal_Date"].map(
        parse_yyyymmdd
    )
    regime_map = (
        history_dates.dropna(subset=["Signal_Date"])
        .groupby("Signal_Date", sort=False)["Market_Regime"]
        .first()
        .to_dict()
    )
    frame["Market_Regime"] = frame["Signal_Date"].map(regime_map).fillna("未知")
    actual_dates = set(
        history_dates.loc[
            _bool_series(history_dates, "Selected_Top2"), "Signal_Date"
        ].dropna().astype(str)
    )
    strong_dates = set(
        history_dates.loc[
            _bool_series(history_dates, "R15_Strong_ATR_Top1"), "Signal_Date"
        ].dropna().astype(str)
    )
    frame["_actual"] = frame["Signal_Date"].astype(str).isin(actual_dates)
    frame["_strong"] = frame["Signal_Date"].astype(str).isin(strong_dates)
    frame["_shadow"] = frame["_actual"] | frame["_strong"]
    rows = []
    for regime in ("强势", "中性", "弱势"):
        group = frame[frame["Market_Regime"].eq(regime)]
        rows.append(
            {
                "市场状态": regime,
                "扫描周": len(group),
                "R3/R6实际信号周": int(group["_actual"].sum()),
                "R15新增强势信号周": int(group["_strong"].sum()),
                "影子组合信号周": int(group["_shadow"].sum()),
                "影子组合新信号覆盖率%": (
                    group["_shadow"].mean() * 100.0 if len(group) else np.nan
                ),
                "影子组合无新信号周": int((~group["_shadow"]).sum()),
            }
        )
    rows.append(
        {
            "市场状态": "合计",
            "扫描周": len(frame),
            "R3/R6实际信号周": int(frame["_actual"].sum()),
            "R15新增强势信号周": int(frame["_strong"].sum()),
            "影子组合信号周": int(frame["_shadow"].sum()),
            "影子组合新信号覆盖率%": frame["_shadow"].mean() * 100.0,
            "影子组合无新信号周": int((~frame["_shadow"]).sum()),
        }
    )
    return pd.DataFrame(rows, columns=columns)


def r15_holding_coverage_audit(history: pd.DataFrame, ledger: pd.DataFrame):
    columns = [
        "固定持有上限", "可观察后续周", "至少一只理论持仓周",
        "理论空仓周", "理论持仓覆盖率%", "计算边界",
    ]
    if history.empty or ledger.empty:
        return pd.DataFrame(columns=columns)
    ...
    actual_dates = set(
        history.loc[
            _bool_series(history, "Selected_Top2"), "Signal_Date"
        ].astype(str)
    )
    strong_dates = set(
        history.loc[
            _bool_series(history, "R15_Strong_ATR_Top1"), "Signal_Date"
        ].astype(str)
    )
    signal_mask = frame["Signal_Date"].astype(str).isin(
        actual_dates | strong_dates
    ).to_numpy()
    ...
    for horizon in R14_LIFECYCLE_HORIZONS:
        ...
        for position in signal_positions:
            active[
                position + 1 : min(len(frame), position + horizon + 1)
            ] = True
        observable = active[1:]
        rows.append(
            {
                "固定持有上限": f"W{horizon}",
                "可观察后续周": len(observable),
                "至少一只理论持仓周": int(observable.sum()),
                "理论空仓周": int((~observable).sum()),
                "理论持仓覆盖率%": observable.mean() * 100.0,
                "计算边界": "忽略三仓冲突、提前退出和区间开始前已有持仓",
            }
        )
    return pd.DataFrame(rows, columns=columns)


def r15_calendar_year_audit(history: pd.DataFrame):
    columns = [
        "年份", "完整交易", "信号周", "胜率%", "中位收益%",
        "平均收益%", "Profit_Factor", "最大单笔亏损%",
    ]
    selected = _r15_strong_rows(history, complete_only=True)
    if selected.empty:
        return pd.DataFrame(columns=columns)
    selected["_year"] = selected["Signal_Date"].astype(str).str[:4]
    rows = []
    for year, group in selected.groupby("_year", sort=True):
        returns = pd.to_numeric(
            group.get(PRIMARY_RETURN_COLUMN), errors="coerce"
        ).dropna()
        rows.append(
            {
                "年份": year,
...
def r15_shadow_acceptance_gates(history: pd.DataFrame, ledger: pd.DataFrame):
    all_selected = _r15_strong_rows(history, complete_only=False)
    selected = _r15_strong_rows(history, complete_only=True)
    returns = pd.to_numeric(
        selected.get(PRIMARY_RETURN_COLUMN, pd.Series(dtype=float)),
        errors="coerce",
    ...
    weeks = all_selected["Signal_Date"].nunique() if len(all_selected) else 0
    if not ledger.empty:
        signal_dates = pd.to_datetime(
            ledger.get("Signal_Date", pd.Series(dtype=str)).map(parse_yyyymmdd),
            format="%Y%m%d",
            errors="coerce",
        ).dropna()
    elif len(all_selected):
        signal_dates = pd.to_datetime(
            all_selected["Signal_Date"].astype(str),
            format="%Y%m%d",
            errors="coerce",
        ).dropna()
    else:
    ...
    if len(signal_dates):
        observed_years = max(
            1.0,
            ((signal_dates.max() - signal_dates.min()).days + 7) / 365.25,
        )
    else:
        observed_years = 1.0
    annualized_weeks = weeks / observed_years
    counts_per_week = (
        all_selected.groupby("Signal_Date").size()
        if len(all_selected)
        else pd.Series(dtype=int)
    )
    chronological = selected.sort_values("Signal_Date")
    split = max(1, len(chronological) // 2) if len(chronological) else 0
    first_returns = pd.to_numeric(
        chronological.iloc[:split].get(PRIMARY_RETURN_COLUMN), errors="coerce"
    ).dropna() if split else pd.Series(dtype=float)
    back_returns = pd.to_numeric(
        chronological.iloc[split:].get(PRIMARY_RETURN_COLUMN), errors="coerce"
    ).dropna() if split else pd.Series(dtype=float)
    ...
    second = history[
        ...
        & pd.to_numeric(
            history.get("R11_Strong_Rank"), errors="coerce"
        ).eq(2)
        & _bool_series(history, "Outcome_Complete")
        & _bool_series(history, "Entry_Tradable")
    ].copy()
    ...
    coverage = r15_signal_coverage_audit(history, ledger)
    total_coverage = coverage[
        coverage.get(
            "市场状态", pd.Series("", index=coverage.index)
        ).astype(str).eq("合计")
    ]
    coverage_rate = _safe_float(
        total_coverage["影子组合新信号覆盖率%"].iloc[0]
        if not total_coverage.empty
        else np.nan
    )
    holding = r15_holding_coverage_audit(history, ledger)
    w3 = holding[
        holding.get(
            "固定持有上限", pd.Series("", index=holding.index)
        ).astype(str).eq("W3")
    ]
    w3_empty = _safe_float(
        w3["理论空仓周"].iloc[0] if not w3.empty else np.nan
    )
    gates = [
        ("R15年化强势信号周位于6至14周", 6.0 <= annualized_weeks <= 14.0, f"当前{annualized_weeks:.1f}周/年"),
        ("R15每个信号周严格只有Top1", len(counts_per_week) > 0 and counts_per_week.eq(1).all(), f"当前{len(all_selected)}笔/{weeks}周"),
        ("R15至少8笔完整强势交易", len(returns) >= 8, f"当前{len(returns)}笔"),
        ("R15 W3胜率至少55%", len(returns) > 0 and (returns > 0.0).mean() >= 0.55, f"当前{((returns > 0.0).mean() * 100.0 if len(returns) else np.nan):.1f}%"),
        ("R15 W3中位收益大于0", len(returns) > 0 and returns.median() > 0.0, f"当前{returns.median() if len(returns) else np.nan:.2f}%"),
        ("R15 Profit Factor至少1.2", pd.notna(_profit_factor(returns)) and _profit_factor(returns) >= 1.2, f"当前{_profit_factor(returns):.2f}"),
        ("R15去最佳一只后平均收益大于0", len(without_best) > 0 and without_best.mean() > 0.0, f"当前{without_best.mean() if len(without_best) else np.nan:.2f}%"),
        ("R15去最佳一只后PF至少1.2", pd.notna(_profit_factor(without_best)) and _profit_factor(without_best) >= 1.2, f"当前{_profit_factor(without_best):.2f}"),
        ("R15最佳一只占正利润不超过40%", math.isfinite(best_contribution) and best_contribution <= 40.0, f"当前{best_contribution:.1f}%"),
        ("R15前后半段中位收益均为正", len(first_returns) > 0 and len(back_returns) > 0 and first_returns.median() > 0.0 and back_returns.median() > 0.0, f"前{first_returns.median() if len(first_returns) else np.nan:.2f}% / 后{back_returns.median() if len(back_returns) else np.nan:.2f}%"),
        ("R15 Top1中位收益高于同周第二名", len(returns) > 0 and len(rank2_returns) > 0 and returns.median() > rank2_returns.median(), f"Top1 {returns.median() if len(returns) else np.nan:.2f}% / 第二名{rank2_returns.median() if len(rank2_returns) else np.nan:.2f}%"),
        ("R15影子组合新信号覆盖率至少70%", math.isfinite(coverage_rate) and coverage_rate >= 70.0, f"当前{coverage_rate:.1f}%"),
        ("R15影子组合固定W3理论空仓不超过5周", math.isfinite(w3_empty) and w3_empty <= 5.0, f"当前{w3_empty:.0f}周"),
    ]
    return pd.DataFrame(
        [
            {"R15验收项目": name, "结果": "通过" if passed else "未通过", "当前值": value}
            for name, passed, value in gates
        ]
    )
...
# -----------------------------------------------------------------------------
# R16 冻结三市场入场后的独立退出与实盘成交审计
# -----------------------------------------------------------------------------
def _r16_selected(history: pd.DataFrame, require_lifecycle: bool = False):
    if history.empty:
        return history.iloc[0:0].copy()
    selected = history[
        _bool_series(history, "R16_Exit_Research_Selected")
        & _bool_series(history, "Entry_Tradable")
    ].copy()
    if require_lifecycle:
        selected = selected[
    ...
    selected["R16_市场分支"] = regime.map(
        {"强势": "R15强势", "中性": "R3中性", "弱势": "R6弱势"}
    ).fillna("未知")
    return selected


def _r16_lifecycle_returns(
    selected: pd.DataFrame,
    horizon: int,
    rule_kind: str,
    trigger_column: str | None,
    exit_return_column: str | None,
    exit_day_column: str | None,
    weekly_exit_base_day: int | None,
):
    fixed_column = f"Fixed_Return_W{horizon}_Net_pct"
    fixed = pd.to_numeric(
        selected.get(fixed_column, pd.Series(np.nan, index=selected.index)),
    ...
    cohort = selected.loc[fixed.notna()].copy()
    fixed = fixed.loc[cohort.index]
    use_exit = pd.Series(False, index=cohort.index, dtype=bool)
    realized = fixed.copy()
    if rule_kind == "fixed" or not trigger_column or not exit_return_column:
        return cohort, realized, use_exit

    trigger = _bool_series(cohort, trigger_column)
    exit_return = pd.to_numeric(
        cohort.get(
            exit_return_column, pd.Series(np.nan, index=cohort.index)
        ),
        errors="coerce",
    )
    if rule_kind == "hard" and exit_day_column:
        exit_day = pd.to_numeric(
            cohort.get(exit_day_column, pd.Series(np.nan, index=cohort.index)),
            errors="coerce",
        )
    else:
        delay_column = (
            "R14_W1_Exit_Delay_Days"
            if weekly_exit_base_day == 6
            else "R14_W2_Exit_Delay_Days"
        )
        exit_day = weekly_exit_base_day + pd.to_numeric(
            cohort.get(delay_column, pd.Series(np.nan, index=cohort.index)),
            errors="coerce",
        )
    use_exit = (
        trigger
        & exit_return.notna()
        & exit_day.le(horizon * MARKET_DAYS_PER_WEEK)
    )
    realized.loc[use_exit] = exit_return.loc[use_exit]
    return cohort, realized, use_exit


def r16_exit_trade_detail(history: pd.DataFrame):
    selected = _r16_selected(history, require_lifecycle=False)
    if selected.empty:
        return selected
    preferred = [
        "Signal_Date", "ts_code", "name", "R16_市场分支",
        "Entry_Date", "Entry_Open", "Outcome_Grade", "Path_10_vs_Minus5",
        "MFE_W3_Net_pct", "MAE_W3_Raw_pct",
        "Fixed_Return_W3_Net_pct", "Fixed_Return_W4_Net_pct",
        "Fixed_Return_W6_Net_pct", "Fixed_Return_W8_Net_pct",
        "R16_Stop_Minus8_Triggered", "R16_Stop_Minus8_Trigger_Date",
        "R16_Stop_Minus8_Exit_Date", "R16_Stop_Minus8_Exit_Day",
        "R16_Stop_Minus8_Return_Net_pct", "R16_Stop_Minus8_Delay_Days",
        "R16_Stop_Minus8_Blocked_Days",
        "R16_Stop_Minus10_Triggered", "R16_Stop_Minus10_Trigger_Date",
        "R16_Stop_Minus10_Exit_Date", "R16_Stop_Minus10_Exit_Day",
        "R16_Stop_Minus10_Return_Net_pct", "R16_Stop_Minus10_Delay_Days",
        "R16_Stop_Minus10_Blocked_Days",
        "R14_Trigger_W1_Close_Loss", "R14_Trigger_W1_W2_Both_Loss",
        "R14_Trigger_W2_Close_Minus5", "R16_Lifecycle_Data_Available",
    ]
    columns = [column for column in preferred if column in selected.columns]
    return selected[columns].sort_values(
        [column for column in ("Signal_Date", "R16_市场分支", "ts_code") if column in columns]
    ).reset_index(drop=True)


def r16_lifecycle_return_audit(history: pd.DataFrame):
    columns = [
        "市场分支", "持有上限", "R16退出规则", "可比样本", "信号周",
        "退出交易数", "退出率%", "胜率%", "中位收益%", "平均收益%",
        "5%截尾均益%", "Profit_Factor", "最大单笔亏损%",
        "相对固定平均改善百分点", "F级捕获率%", "S/A误杀率%",
    ]
    selected = _r16_selected(history, require_lifecycle=True)
    if selected.empty:
        return pd.DataFrame(columns=columns)
    rows = []
    branch_groups = [("合计", selected)] + [
        (label, selected[selected["R16_市场分支"].eq(label)])
        for label in ("R15强势", "R3中性", "R6弱势")
    ]
    for branch, branch_frame in branch_groups:
        for horizon in R16_LIFECYCLE_HORIZONS:
            for (
                label, kind, trigger_column, exit_column,
                exit_day_column, weekly_base_day,
            ) in R16_EXIT_RULES:
                cohort, realized, use_exit = _r16_lifecycle_returns(
                    branch_frame,
                    horizon,
                    kind,
                    trigger_column,
                ...
                realized = pd.to_numeric(realized, errors="coerce").dropna()
                cohort = cohort.loc[realized.index]
                use_exit = use_exit.loc[realized.index]
                fixed = pd.to_numeric(
                    cohort.get(f"Fixed_Return_W{horizon}_Net_pct"),
                    errors="coerce",
                )
                grades = cohort.get(
                    "Outcome_Grade", pd.Series("", index=cohort.index)
                ...
                rows.append(
                    {
                        ...
                        "可比样本": len(realized),
                        "信号周": cohort["Signal_Date"].nunique() if len(cohort) else 0,
                        "退出交易数": int(use_exit.sum()),
                        "退出率%": use_exit.mean() * 100.0 if len(use_exit) else np.nan,
                        "胜率%": (realized > 0.0).mean() * 100.0 if len(realized) else np.nan,
                        "中位收益%": realized.median() if len(realized) else np.nan,
                        "平均收益%": realized.mean() if len(realized) else np.nan,
                        "5%截尾均益%": _trimmed_mean(realized),
                        ...
                        "最大单笔亏损%": realized.min() if len(realized) else np.nan,
                        "相对固定平均改善百分点": (
                            (realized - fixed).mean() if len(realized) else np.nan
                        ),
                        "F级捕获率%": (
                            use_exit[f_mask].mean() * 100.0 if f_mask.any() else np.nan
                        ),
                        "S/A误杀率%": (
                            use_exit[sa_mask].mean() * 100.0 if sa_mask.any() else np.nan
                        ),
                    }
                )
    return pd.DataFrame(rows, columns=columns)
...
def r16_w3_exit_rule_summary(history: pd.DataFrame):
    audit = r16_lifecycle_return_audit(history)
    if audit.empty:
        return audit
    return audit[audit["持有上限"].astype(str).eq("W3")].reset_index(drop=True)


def r16_primary_stop_risk_audit(history: pd.DataFrame):
    columns = [
        "市场分支", "完整样本", "信号周", "-10%触发数", "实际退出数",
        "跌停阻塞交易", "平均退出延迟日", "固定W3胜率%", "止损后胜率%",
        "固定W3中位收益%", "止损后中位收益%", "固定W3平均收益%",
        "止损后平均收益%", "固定W3_PF", "止损后_PF",
        "W3最差收盘收益%", "止损后最差收益%", "W3最差盘中MAE%",
        "相对最差盘中路径改善百分点", "F级捕获率%", "S/A误杀率%",
    ]
    selected = _r16_selected(history, require_lifecycle=True)
    if selected.empty:
        return pd.DataFrame(columns=columns)
    ...
    for branch, group in groups:
        cohort, realized, use_exit = _r16_lifecycle_returns(
            group, 3, "hard", "R16_Stop_Minus10_Triggered",
            "R16_Stop_Minus10_Return_Net_pct",
            "R16_Stop_Minus10_Exit_Day", None,
        )
        realized = pd.to_numeric(realized, errors="coerce").dropna()
        cohort = cohort.loc[realized.index]
        use_exit = use_exit.loc[realized.index]
        fixed = pd.to_numeric(
            cohort.get("Fixed_Return_W3_Net_pct"), errors="coerce"
        )
        mae = pd.to_numeric(cohort.get("MAE_W3_Raw_pct"), errors="coerce")
        ...
        triggered = (
            _bool_series(cohort, "R16_Stop_Minus10_Triggered")
            & trigger_day.le(PRIMARY_HOLD_WEEKS * MARKET_DAYS_PER_WEEK)
        )
        blocked = pd.to_numeric(
            cohort.get(
                "R16_Stop_Minus10_Blocked_Days",
                pd.Series(0, index=cohort.index),
            ), errors="coerce"
        ).fillna(0)
        delays = pd.to_numeric(
            cohort.get(
        ...
        rows.append(
            {
                ...
                "完整样本": len(realized),
                "信号周": cohort["Signal_Date"].nunique() if len(cohort) else 0,
                "-10%触发数": int(triggered.sum()),
                "实际退出数": int(use_exit.sum()),
                "跌停阻塞交易": int((blocked.gt(0) & triggered).sum()),
                "平均退出延迟日": delays[use_exit].mean() if use_exit.any() else np.nan,
                "固定W3胜率%": (fixed > 0.0).mean() * 100.0 if len(fixed) else np.nan,
                "止损后胜率%": (realized > 0.0).mean() * 100.0 if len(realized) else np.nan,
                "固定W3中位收益%": fixed.median() if len(fixed) else np.nan,
                "止损后中位收益%": realized.median() if len(realized) else np.nan,
                "固定W3平均收益%": fixed.mean() if len(fixed) else np.nan,
                "止损后平均收益%": realized.mean() if len(realized) else np.nan,
                "固定W3_PF": _profit_factor(fixed),
                "止损后_PF": _profit_factor(realized),
                "W3最差收盘收益%": fixed.min() if len(fixed) else np.nan,
                "止损后最差收益%": realized.min() if len(realized) else np.nan,
                "W3最差盘中MAE%": mae.min() if len(mae.dropna()) else np.nan,
                "相对最差盘中路径改善百分点": (
                    realized.min() - mae.min()
                    if len(realized) and len(mae.dropna()) else np.nan
                ),
                "F级捕获率%": use_exit[f_mask].mean() * 100.0 if f_mask.any() else np.nan,
                "S/A误杀率%": use_exit[sa_mask].mean() * 100.0 if sa_mask.any() else np.nan,
            }
        )
    return pd.DataFrame(rows, columns=columns)


def r16_exit_acceptance_gates(history: pd.DataFrame):
    risk = r16_primary_stop_risk_audit(history)
    total = risk[risk.get(
        "市场分支", pd.Series("", index=risk.index)
    ).astype(str).eq("合计")]
    total_row = total.iloc[0] if not total.empty else pd.Series(dtype=object)
    selected = _r16_selected(history, require_lifecycle=True)
    branch_counts = (
        selected.groupby("R16_市场分支").size().to_dict()
        if len(selected) else {}
    )
    median_return = _safe_float(total_row.get("止损后中位收益%"))
    primary_pf = _safe_float(total_row.get("止损后_PF"))
    worst_return = _safe_float(total_row.get("止损后最差收益%"))
    path_improvement = _safe_float(
        total_row.get("相对最差盘中路径改善百分点")
    )
    f_capture = _safe_float(total_row.get("F级捕获率%"))
    gates = [
        ("实现约束", "主止损严格固定为-10%", R16_PRIMARY_STOP_PCT == -10.0, f"当前{R16_PRIMARY_STOP_PCT:.1f}%"),
        ("实现约束", "止损仅从买入次日生效（T+1）", True, "已锁定"),
        ("实现约束", "止损成交计0.3%不利滑点", np.isclose(R16_STOP_SLIPPAGE_PCT, 0.30), f"当前{R16_STOP_SLIPPAGE_PCT:.2f}%"),
        ("实现约束", "停牌与一字跌停顺延，不虚构成交", True, "已锁定"),
        ("样本门槛", "R3中性至少24笔完整交易", branch_counts.get("R3中性", 0) >= 24, f"当前{branch_counts.get('R3中性', 0)}笔"),
        ("样本门槛", "R6弱势至少12笔完整交易", branch_counts.get("R6弱势", 0) >= 12, f"当前{branch_counts.get('R6弱势', 0)}笔"),
        ("样本门槛", "R15强势至少8笔完整交易", branch_counts.get("R15强势", 0) >= 8, f"当前{branch_counts.get('R15强势', 0)}笔"),
        ("风险效果", "主止损W3中位收益仍大于0", math.isfinite(median_return) and median_return > 0.0, f"当前{median_return:.2f}%"),
        ("风险效果", "主止损W3 Profit Factor至少1.2", math.isfinite(primary_pf) and primary_pf >= 1.2, f"当前{primary_pf:.2f}"),
        ("风险效果", "主止损最差实际收益不低于-15%", math.isfinite(worst_return) and worst_return >= -15.0, f"当前{worst_return:.2f}%"),
        ("风险效果", "相对最差盘中路径至少改善3个百分点", math.isfinite(path_improvement) and path_improvement >= 3.0, f"当前{path_improvement:.2f}个百分点"),
        ("风险效果", "主止损捕获至少50%的F级", math.isfinite(f_capture) and f_capture >= 50.0, f"当前{f_capture:.1f}%"),
    ]
    return pd.DataFrame(
        [
            {
...
def _apply_r16_selection_policy(frame: pd.DataFrame):
    ...
    result["R15_Strong_ATR_Top1"] = _bool_series(
        result, "R11_Strong_Research_Top1"
    )
    ...
    result["R15_Shadow_Selected"] = (
        _bool_series(result, "Selected_Top2")
        | _bool_series(result, "R15_Strong_ATR_Top1")
    )
    result["R16_Exit_Research_Selected"] = _bool_series(
        result, "R15_Shadow_Selected"
    )
    return result
...
def import_r16_results_zip(zip_bytes: bytes, config_id: str):
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes)) as archive:
        ...
        candidates = _apply_r16_selection_policy(candidates)
        ...
        r16_lifecycle_required = {
            "R16_Stop_Minus8_Triggered",
            "R16_Stop_Minus8_Trigger_Day",
            "R16_Stop_Minus8_Return_Net_pct",
            "R16_Stop_Minus8_Exit_Day",
            "R16_Stop_Minus8_Delay_Days",
            "R16_Stop_Minus8_Blocked_Days",
            "R16_Stop_Minus10_Triggered",
            "R16_Stop_Minus10_Trigger_Day",
            "R16_Stop_Minus10_Return_Net_pct",
            ...
            "R16_Stop_Minus10_Delay_Days",
            "R16_Stop_Minus10_Blocked_Days",
            "R16_Lifecycle_Data_Available",
        }
        legacy_missing_r16 = not r16_lifecycle_required.issubset(
            candidates.columns
        )
        pending_r16_dates = set(
            candidates.loc[
                _bool_series(candidates, "R16_Exit_Research_Selected"),
                "Signal_Date",
            ].astype(str)
        ) if legacy_missing_r16 else set()
        ...
        if opportunity_name:
            ...
            opportunities = _apply_r16_selection_policy(opportunities)
            ...
            opportunities["R15_Strong_ATR_Top1"] = _bool_series(
                opportunities, "R15_Strong_ATR_Top1"
            )
            opportunities["R15_Shadow_Selected"] = _bool_series(
                opportunities, "R15_Shadow_Selected"
            )
            opportunities["R16_Exit_Research_Selected"] = _bool_series(
                opportunities, "R16_Exit_Research_Selected"
            )
        ...
        if not legacy_missing_r13 and pending_r16_dates:
            r16_pending_mask = (
                imported_ledger["Signal_Date"].astype(str).isin(
                    pending_r16_dates
                )
                & ~imported_ledger.get(
            ...
            imported_ledger.loc[
                r16_pending_mask, "Scan_Status"
            ] = "PENDING_R16_EXIT"
            imported_ledger.loc[
                r16_pending_mask, "Selection_Block_Reason"
            ] = "冻结入场已恢复；等待补算R16 T+1日内硬止损成交路径"
    ...
    return {
        ...
        "pending_r16_weeks": len(pending_r16_dates),
        "r15_strong_weeks": int(
            candidates.loc[
                _bool_series(candidates, "R15_Strong_ATR_Top1"),
                "Signal_Date",
            ].nunique()
...
def build_export_zip(
    ...
    r15_strong_audit: pd.DataFrame,
    r15_shadow_performance: pd.DataFrame,
    r15_signal_coverage: pd.DataFrame,
    r15_holding_coverage: pd.DataFrame,
    r15_yearly_audit: pd.DataFrame,
    r15_gates: pd.DataFrame,
    r16_trade_detail: pd.DataFrame,
    r16_w3_exit_summary: pd.DataFrame,
    ...
    files = {
        ...
        "46_r15_strong_atr_top1_candidate_audit.csv": r15_strong_audit,
        "47_r15_shadow_combined_performance.csv": r15_shadow_performance,
        "48_r15_signal_coverage_audit.csv": r15_signal_coverage,
        "49_r15_fixed_holding_coverage_audit.csv": r15_holding_coverage,
        "50_r15_calendar_year_strong_audit.csv": r15_yearly_audit,
        "51_r15_shadow_acceptance_gates.csv": r15_gates,
        "52_r16_exit_trade_detail.csv": r16_trade_detail,
        "53_r16_w3_exit_rule_summary.csv": r16_w3_exit_summary,
...
- **R15唯一强势方法**：完全复用R11；强势周在再启动合格池中按ATR3/ATR13从小到大排名，只检查Top1且ATR比必须位于0.70—0.90。
- **R15禁止递补**：第一名不在接受带时直接空仓，绝不改用第二名；R7、R9失败方案不得拼接补周。
- **R15影子组合**：中性仍用R3 Top2、弱势仍用R6 Top2；强势只加入R15 Top1，独立报告新信号覆盖、固定W3理论空窗与W3收益。
- **R15实际隔离**：影子标记不覆盖Selected_Top2，不参与原R3/R6实际组合统计；通过后仍需样本外确认。
- **R16冻结入场**：R3中性Top2、R6弱势Top2、R15强势Top1逐行不变；每笔交易独立评价，暂不建立三仓组合。
- **R16主止损**：买入后从下一交易日起检查日内-10%；-8%只作敏感性对照，不加入更宽松的-12%方案。
...
- **研究隔离**：R11、R12、R13、R14、R15以及R7/R9对照均使用独立标记，永远不能进入R3/R6实际收益或持股统计。
...
- **明确排除**：买入后的走势、止损、止盈、移动保护、S/A/B/F结果均不参与入场评分。
    ...
    if import_results_clicked and imported_results is not None:
        try:
            import_stats = import_r16_results_zip(
            ...
            r15_note = (
                f"；已恢复R15强势影子信号{import_stats.get('r15_strong_weeks', 0)}周"
            )
            r16_note = (
                f"；其中{import_stats.get('pending_r16_weeks', 0)}个冻结信号周需补算R16硬止损路径"
    ...
    if "r16_worker_id" not in st.session_state:
        st.session_state["r16_worker_id"] = uuid.uuid4().hex
    worker_id = str(st.session_state["r16_worker_id"])
            ...
            try:
                ...
                if not pending_dates:
                    ...
                    for idx, signal_date in enumerate(batch_dates):
                        ...
                        if run_preview:
                            st.session_state["r16_preview"] = candidates
                        else:
                            if not candidates.empty:
    ...
    preview = st.session_state.get("r16_preview")
    if is_preview_mode and isinstance(preview, pd.DataFrame):
        ...
        if preview.empty:
            ...
            r15_preview = preview[
                _bool_series(preview, "R15_Strong_ATR_Top1")
            ].copy()
            if not r15_preview.empty:
                st.caption(
                    "以下是R15强势温和ATR Top1影子信号：只检查第一名且ATR比位于"
                    "0.70—0.90；仍不属于原R3/R6实际买入名单。"
                )
                r15_columns = [
                    "Signal_Date", "R11_Strong_Rank", "name", "ts_code",
                    "Industry", "ATR_Contraction", "Strong_Reacceleration_Setup_Type",
                    "Daily_MACD_Hist_Delta_pct", "Return_1W_pct",
                    "Distance_MA20_pct", "R15_Strong_ATR_Top1",
                    "R15_Shadow_Selected",
                ]
                st.dataframe(
                    r15_preview[
                        [column for column in r15_columns if column in r15_preview.columns]
                    ],
                    width="stretch",
                    hide_index=True,
                )
    ...
    if not raw_history.empty:
        ...
        pending_r16_rows = data_gap_rows[
            data_gap_rows.get(
                "Scan_Status", pd.Series("", index=data_gap_rows.index)
            ).astype(str).eq("PENDING_R16_EXIT")
        ].copy()
        actual_data_gap_rows = data_gap_rows[
            ~data_gap_rows.index.isin(
                pending_r13_rows.index.union(pending_r14_rows.index).union(
                    pending_r16_rows.index
                )
            )
        ].copy()
        ...
        r15_strong_audit = r11_audit.copy()
        r15_shadow_performance = r15_shadow_performance_audit(history)
        r15_signal_coverage = r15_signal_coverage_audit(history, ledger)
        r15_holding_coverage = r15_holding_coverage_audit(history, ledger)
        r15_yearly_audit = r15_calendar_year_audit(history)
        r15_gates = r15_shadow_acceptance_gates(history, ledger)
        r16_trade_detail = r16_exit_trade_detail(history)
        r16_w3_exit_summary = r16_w3_exit_rule_summary(history)
        ...
        pending_r16_weeks = int(ledger_status.eq("PENDING_R16_EXIT").sum())
        data_gap_weeks = int(
            (
        ...
        metric_columns[3].metric("待补R16止损周", pending_r16_weeks)
        metric_columns[4].metric("数据缺口周", data_gap_weeks)
        metric_columns[5].metric("跳过扫描周", skipped_data_weeks)
        metric_columns[6].metric("无有效入选周", invalid_selection_weeks)
        metric_columns[7].metric("W3完整入选交易", len(selected))
        ...
        r15_total_coverage = r15_signal_coverage[
            r15_signal_coverage.get(
                "市场状态", pd.Series("", index=r15_signal_coverage.index)
            ).astype(str).eq("合计")
        ]
        r15_w3_coverage = r15_holding_coverage[
            r15_holding_coverage.get(
                "固定持有上限", pd.Series("", index=r15_holding_coverage.index)
            ).astype(str).eq("W3")
        ]
        r15_metric_columns = st.columns(4)
        r15_metric_columns[0].metric(
            "R15新增强势信号周",
            int(r15_total_coverage["R15新增强势信号周"].iloc[0])
            if not r15_total_coverage.empty
            else 0,
        )
        r15_metric_columns[1].metric(
            "影子组合新信号周",
            int(r15_total_coverage["影子组合信号周"].iloc[0])
            if not r15_total_coverage.empty
            else 0,
        )
        r15_metric_columns[2].metric(
            "影子组合新信号覆盖率",
            f"{_safe_float(r15_total_coverage['影子组合新信号覆盖率%'].iloc[0]) if not r15_total_coverage.empty else np.nan:.1f}%",
        )
        r15_metric_columns[3].metric(
            "固定W3理论空仓周",
            int(r15_w3_coverage["理论空仓周"].iloc[0])
            if not r15_w3_coverage.empty
            else 0,
            help="只检验信号时间覆盖；忽略三仓上限、提前退出和区间开始前已有持仓。",
        )
        ...
        if not pending_r16_rows.empty:
            st.info(
                f"已恢复冻结入场结果，其中{len(pending_r16_rows)}个信号周尚缺R16硬止损路径；"
                "启动R16后只补扫这些日期，不会重做其他已完成周。"
            )
        ...
        st.caption(
            "主规则始终是日内-10%硬止损；S/A误杀率只揭示代价，不作为放宽止损的门槛。"
        )
        if _bool_series(history, "R16_Lifecycle_Data_Available").sum() == 0:
            st.info(
                "当前结果包尚无R16日内路径。导入后启动历史验证，程序只补算冻结信号周。"
            )
        st.dataframe(r16_gates, width="stretch", hide_index=True)

        st.subheader("R16三市场-10%主止损风险效果")
        st.caption(
            "最差盘中MAE是实际持有过程风险；止损后最差收益包含跳空、0.3%滑点和跌停延迟。"
        )
        st.dataframe(
            _format_report_frame(r16_primary_risk),
            width="stretch",
            hide_index=True,
        )

        st.subheader("R16 W3固定持有、硬止损与周末退出对照")
        st.dataframe(
            _format_report_frame(r16_w3_exit_summary),
            width="stretch",
            hide_index=True,
        )

        with st.expander("查看R16 W3/W4/W6/W8全部退出生命周期"):
            st.dataframe(
                _format_report_frame(r16_lifecycle_audit),
                width="stretch",
                hide_index=True,
            )
        with st.expander("查看R16逐笔止损成交路径"):
            st.dataframe(
                _format_report_frame(r16_trade_detail),
                width="stretch",
                hide_index=True,
            )
        ...
        st.subheader("R15强势温和ATR Top1与影子组合验收（只研究）")
        st.caption(
            "强势分支保持单一方法，不新增第二套强市引擎；13项门槛同时约束信号数量、"
            "收益稳健性、Top1排名有效性和全年时间覆盖。"
        )
        st.dataframe(r15_gates, width="stretch", hide_index=True)
        st.subheader("R15三类市场新信号覆盖")
        st.dataframe(
            _format_report_frame(r15_signal_coverage),
            width="stretch",
            hide_index=True,
        )
        st.subheader("R15影子组合固定持有时间覆盖")
        st.caption(
            "这是时间轴覆盖审计，不是组合收益回测；入选后的下一周视为开始持有。"
        )
        st.dataframe(
            _format_report_frame(r15_holding_coverage),
            width="stretch",
            hide_index=True,
        )
        st.subheader("R15影子组合W3收益对照")
        st.dataframe(
            _format_report_frame(r15_shadow_performance),
            width="stretch",
            hide_index=True,
        )
        st.subheader("R15强势分支分年表现")
        st.dataframe(
            _format_report_frame(r15_yearly_audit),
            width="stretch",
            hide_index=True,
        )
        ...
        all_r15_passed = (
            not r15_gates.empty and r15_gates["结果"].eq("通过").all()
        )
        if (
            all_overall_passed
            ...
            st.success(
                "R3/R6原实际分支与R15强势影子分支全部通过当前区间门槛；"
                "R15仍不写入实际组合，下一步只做样本外确认。"
            )
        elif all_overall_passed and all_neutral_passed and all_recovery_passed:
            st.warning(
                "R3/R6原实际分支通过，但R15强势影子分支未全部通过；"
                "强势周继续按原规则空仓，不为补周调整阈值。"
            )
        else:
            st.error("R3、R6或总体门槛尚未全部通过；R11—R15研究信号均禁止进入实盘。")
        ...
        with st.expander("查看R15强势温和ATR Top1影子信号明细"):
            r15_detail_columns = [
                "Signal_Date", "Entry_Date", "name", "ts_code", "Industry",
                "R11_Strong_Rank", "ATR_Contraction",
                "R11_ATR_Band_Pass", "Strong_Reacceleration_Eligible",
                "Strong_Reacceleration_Setup", "Entry_Open",
                "Fixed_Return_W1_Net_pct", "Fixed_Return_W2_Net_pct",
                "Fixed_Return_W3_Net_pct", "Fixed_Return_W4_Net_pct",
                "Fixed_Return_W6_Net_pct", "Fixed_Return_W8_Net_pct",
                "MFE_W3_Net_pct", "MAE_W3_Raw_pct", "Outcome_Grade",
                "Outcome_Complete", "R15_Strong_ATR_Top1",
                "R15_Shadow_Selected",
            ]
            r15_detail = history[
                _bool_series(history, "R15_Strong_ATR_Top1")
            ].copy()
            r15_detail = r15_detail.sort_values(
                ["Signal_Date", "R11_Strong_Rank"],
                ascending=[False, True],
                kind="mergesort",
            )
            st.dataframe(
                r15_detail[
                    [
                        column
                        for column in r15_detail_columns
                        if column in r15_detail.columns
                    ]
                ],
                width="stretch",
                hide_index=True,
        ...
        export_bytes = build_export_zip(
            ...
            r14_gates,
            r15_strong_audit,
            r15_shadow_performance,
            r15_signal_coverage,
            r15_holding_coverage,
            r15_yearly_audit,
            r15_gates,
            r16_trade_detail,
            r16_w3_exit_summary,
